...
// Display
function toggleDisplay(visivel) {
    elements.article.style.display = visivel ? "flex" : "none";
    elements.backForma.style.display = visivel ? "flex" : "none";
    document.querySelector('body').style.overflow = visivel ? "hidden" : "scroll";
...
elements.maisInforma.onclick = function (){
    elements.textoForma.classList.toggle(`comTexto`);
    elements.maisInforma.innerHTML = elements.textoForma.classList == "textoForma" ? "<h2>Saiba mais da figura</h2>" : "<h2>Voltar ao cálculo</h2>"
}
...
var formaGeral = [
    {
        ...
        formula: "π × r²",
        texto: descriForma[0],
        area: function(a) { return `≈` + ((Math.PI * (a ** 2))).toFixed(2)}
    },
    {
        nome: 'cubo',
        nume: 0,
    ...
    },
    {
        ...
        texto: descriForma[3],
        area: function(a, b, c) { return (2 * a * b) + (2 * a * c) + (2 * b * c)}
    },
    {
        nome: 'trapezoide',
        nume: 2,
        formula: "(B + b) × h /2",
        texto: descriForma[4],
        area: function(a, b, c) { return (a + b) * c /2}
    },
    {
        nome: 'poligono',
        nume: 2,
        formula: "(n × l /2) × α",
        texto: descriForma[5],
        area: function(a, b, c) { return (a * b /2) * c}
    },
    {
        nome: 'quadrado',
        nume: 0,
